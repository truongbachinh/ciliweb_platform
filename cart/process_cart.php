...
if (isset($_SESSION['current_user']) && $_SESSION["current_user"]['user_role_id'] == '2' &&  $_SESSION["current_user"]['user_status'] == '1') {
    switch ($_GET['view']) {
        case "add_to_cart":
            if (isset($_POST["quantity"]) && $cartUserId) {
                ...
                foreach ($cartProduct as $key => $product) {
                    ...
                    $insertString .= "(NULL, '" . $cartUserId . "', '" . $product['p_id'] . "', '" . $_POST['quantity'][$product["p_id"]] . "', '" . $timeInVietNam . "')";
                ...
                foreach ($_POST['quantity'] as $id => $quantity) {
                    ...
                    if ($cartId) {
                        ...
                        $cartUpdate = mysqli_query($link, "UPDATE `cart` SET `cart_quantity` = '" . $amount . "', `cart_update_time` = '" . $timeInVietNam . "' where cart_id = '$cartId'");
                        echo json_encode(
                            array(
                                'status' => $cartUpdate,
        ...
        case "update_cart";
            if (isset($_POST["quantity"])) {
                foreach ($_POST['quantity'] as $id => $quantity) {
                    $cartUpdate = mysqli_query($link, "UPDATE `cart`  SET `cart_quantity`  = '" . $quantity . "', `cart_update_time` = '" . $timeInVietNam . "'  where cart_user_id = '$cartUserId' AND  cart_id = $id");
                }
                echo json_encode(array(
                    'status' => $cartUpdate,
